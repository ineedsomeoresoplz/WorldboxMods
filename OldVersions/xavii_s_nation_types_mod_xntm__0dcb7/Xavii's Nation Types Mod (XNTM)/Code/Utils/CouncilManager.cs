...
namespace XNTM.Code.Utils
{
    ...
    public static class CouncilManager
    {
        ...
        public static void TickCouncil(Kingdom kingdom)
        {
            ...
            if (kingdom.data.timer_new_king > 0f)
                return;
            ...
            {
                int targetSeats = DetermineTargetSeats(electableCount, out bool shouldBeTemporary);
        ...
        public static void OnActorDied(Actor actor, Kingdom kingdomBeforeDeath, Actor killer)
        {
            ...
            bool temporary = UpdateTemporaryFlag(kingdomBeforeDeath, members);

            LogCouncilDeath(kingdomBeforeDeath, actor, killer);
        ...
        #region Core helpers
        private static int DetermineTargetSeats(int electableCount, out bool temporary)
        {
        ...
        private static void ElectMembers(Kingdom kingdom, List<Actor> members, int targetSeats, bool temporary, ListPool<Actor> electables)
        {
            ...
            foreach (Actor candidate in electables.OrderByDescending(a => BuildCouncilScore(kingdom, a)))
            {
                ...
                LogElection(kingdom, candidate, members.Count, targetSeats, temporary);
                if (members.Count >= targetSeats)
                    break;
            }
        }
        ...
        private static void LogCouncilDeath(Kingdom kingdom, Actor actor, Actor killer)
        {
            if (kingdom == null || actor == null)
                return;

            string title = BuildCouncilorTitle(kingdom, actor);
        ...
        private static string BuildCouncilorTitle(Kingdom kingdom, Actor actor)
        {
            long id = kingdom?.getID() ?? -1;
            string baseTitle = "Councillor";
            if (_infoCache.TryGetValue(id, out var info) && info.Temporary)
                baseTitle = "Acting Councillor";
