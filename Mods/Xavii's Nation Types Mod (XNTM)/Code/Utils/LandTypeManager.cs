...
namespace XNTM.Code.Utils
{
    ...
    public static class LandTypeManager
    {
        ...
        public static void ApplyStatusAdjustments(City city)
        {
            ...
            int population = GetPopulationEstimate(city);
            int housingCap = Mathf.Max(0, Mathf.FloorToInt(population * 0.9f));
            int militiaCap = Mathf.Max(0, Mathf.FloorToInt(population / 3f));

            city.status.houses_max = Mathf.Clamp(city.status.houses_max, 0, housingCap);
            city.status.warrior_slots = Mathf.Clamp(city.status.warrior_slots, 0, militiaCap);
        }
        ...
        public static int ApplyLoyaltyModifier(City city, int loyalty)
        {
            ...
            float support = NationTypeManager.GetCitySupportForCurrentType(city);
            if (support >= 0.62f)
                loyalty += 2;
            else if (support >= 0.54f)
                loyalty += 1;
            else if (support <= 0.33f)
                loyalty -= 2;
            else if (support <= 0.44f)
                loyalty -= 1;
            return loyalty;
        }
        ...
        {
            float baseMultiplier = def.BuildSpeedMultiplier + GetSuccessionBuildModifier(def, nation) + GetNationBuildModifier(def, nation);
            float energyMultiplier = GetEnergyBuildMultiplier(city);
            float landMultiplier = GetLandBuildMultiplier(city);
            float multiplier = Mathf.Max(0f, baseMultiplier * energyMultiplier * landMultiplier);
        ...
        private static float GetEnergyBuildMultiplier(City city)
        {
            if (city == null || city.units == null || city.units.Count == 0)
                return 1f;
            ...
            for (int i = 0; i < city.units.Count; i++)
            {
                ...
                int max = actor.getMaxStamina();
                if (max <= 0)
                    continue;

                total += Mathf.Clamp01(actor.getStamina() / (float)max);
                counted++;
                if (counted >= 90)
                    break;
            }
            ...
            return Mathf.Lerp(0.72f, 1.28f, average);
        }

        private static float GetLandBuildMultiplier(City city)
        {
            if (city == null)
                return 1f;
            int zoneCount = city.zones?.Count ?? 0;
            float normalized = Mathf.Clamp01((zoneCount - 4f) / 48f);
            return Mathf.Lerp(0.8f, 1.3f, normalized);
        }
        ...
        private static int GetNationLoyaltyAdjustment(LandTypeDefinition land, NationTypeDefinition nation)
        {
            if (nation == null)
                return 0;
            if (nation.Id == "military_government")
            {
                if (land.Id == "village")
                    return 3;
                if (land.Id == "city")
        ...
        private static float GetNationBuildModifier(LandTypeDefinition land, NationTypeDefinition nation)
        {
            if (nation == null)
                return 0f;
            if (nation.Id != "council_republic")
